...
function Filter(props) {
  ...
  const { city, country, guests, setCity, setCountry, setGuests } = useContext(
    FilterContext
  );
  ...
  const displayLocations = () => {
    ...
    stays.forEach((stay) => {
      if (
        locations.find(
          (location) =>
            location.city === stay.city && location.country === stay.country
        ) === undefined
      ) {
  ...
  return (
    <FilterMenuWrapper>
      <CancelIcon
        style={{ cursor: "pointer", fontSize: "28px", margin: "0.5rem" }}
        onClick={() => props.toggleMenu((state) => !state)}
      />
      <FilterMenu>
        <InputWrapper>
          ...
          {showLocations
            ? locations.map((location, i) => (
                <Location
                  key={i}
                  onClick={() => {
                    setLoc(`${location.city}, ${location.country}`);
                    setshowLocations(false);
                  }}
                >
                  <RoomIcon /> {location.city}, {location.country}
                </Location>
              ))
            : null}
        </div>
